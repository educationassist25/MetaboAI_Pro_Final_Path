...
def pca_score_plot(pca, scores_df: pd.DataFrame, meta: pd.DataFrame,
    ...
    ax.set_title(f"PCA Score Plot (colored by {group_col})",
...
def pca_loading_plot(pca, feature_names, top_n: int = 20):
    ...
    ax.set_title(f"PCA Loading Plot (top {top_n} metabolites)",
                 fontsize=10.5, fontweight="bold", loc="center")
    fig.tight_layout()
    return fig, top
...
def pca_variance_plot(pca):
    ...
    ax.set_title(f"Explained Variance (Top-2: {var[:2].sum():.1f}%)",
                 fontsize=10.5, fontweight="bold", loc="center")
