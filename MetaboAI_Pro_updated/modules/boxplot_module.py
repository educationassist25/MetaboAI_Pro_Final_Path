...
def boxplot_metabolites(
    ...
    fig_w = fig_width_in or max(3.4, 2.9 * ncols)
    fig_h = fig_height_in or max(3.0, 2.7 * nrows)
    ...
    for idx, met in enumerate(metabolites):
        ...
        ax.set_title(met, fontsize=font_size + 1, fontweight="bold", loc="center", fontfamily=font_family)
        ...
        if stats_table is not None and met in stats_table.index:
            ...
            txt = f"p = {p:.3g}" + (f"\n{fdr_label} = {fdr:.3g}" if not pd.isna(fdr) else "")
    ...
    # Hide any unused subplot cells. set_visible(False) (not just axis('off')) is
    # important here: it also excludes these empty cells from the tight bounding
    # box computed at export time (bbox_inches='tight' in export_figure below).
    # Without it, an incomplete last row (e.g. 2 metabolites with 3 panels/row)
    # leaves blank canvas on the right of the exported image, making the actual
    # boxplot panels look shoved to the left instead of centered.
