...
def _text_width_in(text: str, fontsize: float) -> float:
    ...
    Explicitly wraps the figure in its own FigureCanvasAgg rather than using
    `tmp_fig.canvas.get_renderer()` directly: that method only exists on Agg-family canvases,
    and on some deployments (e.g. Streamlit Community Cloud) something can select a different
    backend before this module's `matplotlib.use("Agg")` takes effect, leaving the figure's
    default canvas without a `get_renderer` method at all -- raising
    `AttributeError: 'FigureCanvasBase' object has no attribute 'get_renderer'` on every rerun.
    """
...
def clustered_heatmap(data_log2: pd.DataFrame, significant_features, meta: pd.DataFrame = None,
    ...
    fig.suptitle(title, y=1.0 - (title_h_in * 0.4) / H, fontsize=font_size + 1,
                 fontweight="bold", ha="center", fontfamily=font_family)
